...
for counter, file_path in enumerate(folder):
    ...
    if('.png' in file_path):
        ...
        '''
        img = rsz(img, 64,64)
        np.save(aug_images[aug_num] + file_path.split("/")[-1][:-4]+".npy", img)
        img_rot90 = rot(img, 90)
        np.save(aug_images[aug_num] + file_path.split("/")[-1][:-4]+"_rot90.npy", img_rot90)
        img_rot270 = rot(img, 270)
        np.save(aug_images[aug_num] + file_path.split("/")[-1][:-4]+"_rot270.npy", img_rot270)
        img_hflip = hflip(img)
        np.save(aug_images[aug_num] + file_path.split("/")[-1][:-4]+"_hflip.npy", img_hflip)
        img_vflip = vflip(img)
        np.save(aug_images[aug_num] + file_path.split("/")[-1][:-4]+"_vflip.npy", img_vflip)
        img_dflip = dflip(img)
        np.save(aug_images[aug_num] + file_path.split("/")[-1][:-4]+"_dflip.npy", img_dflip)
        img_gauss = gauss(img, 10)
        np.save(aug_images[aug_num] + file_path.split("/")[-1][:-4]+"_gauss10.npy", img_gauss)
